...
# Clase Contacto (TDA)
class Contacto:
    def __init__(self, nombre, primer_apellido, segundo_apellido, telefono, correo, direccion, categoria, especificar_otro=""):
        self.nombre = nombre
        ...
        self.especificar_otro = especificar_otro
...
def agregar_contacto(event=None):
    ...
    # Mostrar ventana emergente para "Otros"
    if categoria == "Otros":
        especificar_otro = mostrar_ventana_especificar_otro()
        if not especificar_otro:
            return  # Si no se especifica, no agregar el contacto
    else:
        especificar_otro = ""

    contacto = Contacto(nombre, primer_apellido, segundo_apellido, telefono, correo, direccion, categoria, especificar_otro)
...
def mostrar_ventana_especificar_otro():
    ventana_otro = Toplevel(ventana)
    ventana_otro.title("Especificar Tipo")
    ventana_otro.configure(bg="#FFB3B3")

    label_otro = tk.Label(ventana_otro, text="Especificar tipo de contacto (por ejemplo, 'Proveedor', 'Compañero de trabajo'):", font=("Montserrat", 10), bg="#FFB3B3")
    label_otro.grid(row=0, column=0, padx=10, pady=10)

    entry_otro = tk.Entry(ventana_otro, width=30, font=("Arial", 10))
    entry_otro.grid(row=1, column=0, padx=10, pady=10)

    def aceptar():
        especificar_otro = entry_otro.get().strip()
        if not especificar_otro:
            messagebox.showerror("Error", "El campo es obligatorio para los contactos en la categoría 'Otros'.")
        else:
            ventana_otro.destroy()
            return especificar_otro

    boton_aceptar = tk.Button(ventana_otro, text="Aceptar", command=aceptar, font=("Montserrat", 10))
    boton_aceptar.grid(row=2, column=0, padx=10, pady=10)

    ventana_otro.wait_window()  # Espera a que se cierre la ventana

    # Si no se completó correctamente, no continuar
    if entry_otro.get().strip() == "":
        return None

    return entry_otro.get().strip()
...
def actualizar_lista_contactos():
    ...
    for contacto in contactos:
        listbox_contactos.insert(tk.END, f"{contacto.nombre} {contacto.primer_apellido} - {contacto.categoria}")
...
def ver_informacion():
    ...
    if contacto.categoria == "Otros" and contacto.especificar_otro:
        label_especificar_otro = tk.Label(ventana_info, text=f"Especificar:", font=fuente_titulo, bg="#FFB3B3")
        label_especificar_otro.grid(row=7, column=0, padx=10, pady=5, sticky="w")
        label_especificar_otro_info = tk.Label(ventana_info, text=contacto.especificar_otro, font=fuente_texto, bg="#FFB3B3")
        label_especificar_otro_info.grid(row=7, column=1, padx=10, pady=5, sticky="w")
...
def editar_contacto():
    ...
    categoria_var.set(contacto.categoria)
    entry_otro.delete(0, tk.END)
    entry_otro.insert(0, contacto.especificar_otro)
...
categoria_var = tk.StringVar(value="Amigos")
combo_categoria = tk.OptionMenu(ventana, categoria_var, "Amigos", "Familiares", "Otros")
combo_categoria.grid(row=6, column=1, padx=10, pady=5)

# Especificar tipo (solo para "Otros")
label_otro = tk.Label(ventana, text="Especificar (si aplica):", font=fuente_etiquetas, bg="#FFB3B3", fg="black")
